...
public class LoggerAspect {
    ...
    @Around("execution(* com.example.service776s.*.*(..))")
    public void log(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        logger.info(joinPoint.getSignature().toString() + " method executuion end");
    }
    ...
    public void logException(JoinPoint joinPoint, Exception ex) {
        logger.log(Level.SEVERE, joinPoint.getSignature() + " An exception thrown with the help of" +
                " @AfterThrowing whic happened due to : " + ex.getMessage());
    }
